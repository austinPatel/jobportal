...
class Training_Search extends CI_Controller {
	...
	public function index()
	{
		...
		//Pagination starts
		$total_rows = $this->posted_training_model->count_searched_training_records($param,$param2);
		$config = pagination_configuration(base_url("search-training/".$this->uri->segment(2)), $total_rows, 20, 3, 5, true);
		...
		$obj_result = $this->posted_training_model->get_searched_training($param,$param2, $config["per_page"], $page);
		...
		$data['title'] = $param.$searched_title.' certification';
		$this->load->view('Training_search_view',$data);
	}
	...
	public function search()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			redirect(base_url('certification_search'),'');
			return;
		}
		...
		redirect(base_url('search-certification/'.$param.'/'.$param2),'');
