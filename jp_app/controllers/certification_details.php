...
class Certification_Details extends CI_Controller {
	...
	public function index()
	{
		...
		$row_posted_job = $this->posted_certification_model->get_active_posted_certification_by_id($job_id);
		if(!$row_posted_job){
			$this->load->view('404_view',$data);	
			return;
		}
		$currently_opened_jobs = $this->posted_certification_model->count_active_opened_certification_by_company_id($row_posted_job->CID);
	
		$company_logo = ($row_posted_job->company_logo)?$row_posted_job->company_logo:'no_logo.jpg';
		if(!$row_posted_job){
			redirect(base_url(),'');
			exit;
		}
		...
		if($this->session->userdata('is_user_login')==TRUE){
			$is_already_applied = $this->applied_certification_model->get_applied_job_by_seeker_and_job_id($this->session->userdata('user_id'), $job_id);
		
			$is_already_applied = ($is_already_applied>0)?'yes':'no';
				// print_r($is_already_applied);
		...
		$this->load->view('job_certification_view',$data);
	}
