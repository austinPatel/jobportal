...
class Voulnteer_Search extends CI_Controller {
	...
	public function index()
	{
		...
		//Pagination starts
		$total_rows = $this->posted_voulnteer_model->count_searched_voulnteer_records($param,$param2);
	 
		$config = pagination_configuration(base_url("search-voulnteer/".$this->uri->segment(2)), $total_rows, 20, 3, 5, true);
		...
		$obj_result = $this->posted_voulnteer_model->get_searched_voulnteer($param,$param2, $config["per_page"], $page);
		...
		$this->load->view('voulnteer_search_view',$data);
	}
	...
	public function search()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			redirect(base_url('voulnteer_search'),'');
			return;
		}
		...
		redirect(base_url('search-voulnteer/'.$param.'/'.$param2),'');
