...
class Post_New_Training extends CI_Controller {
	...
	public function index()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$this->load->view('training/post_new_training_view',$data);
			return;
		...
		$job_id = $this->posted_training_model->add_posted_job($job_array);
		$job_slug = $this->make_job_slug($row->company_slug, $this->input->post('job_title'), $this->input->post('city'), $job_id);
		$this->posted_training_model->update_posted_job($job_id, array('job_slug' => $job_slug));
		...
		redirect(base_url('training/my_posted_training'),'');
