<?php
class Posted_Voulnteer_Model extends CI_Model {
	...
	public function add_posted_job($data){
            ...
            $return = $this->db->insert('pp_post_voulnteer', $data);
            if ((bool) $return === TRUE) {
	...
	public function update_posted_job($id, $data){
		$this->db->where('ID', $id);
		$return=$this->db->update('pp_post_voulnteer', $data);
		return $return;
	}
	
	public function delete_posted_job($id){
		$this->db->where('ID', $id);
		$this->db->delete('pp_post_voulnteer');
	}
	...
	public function delete_posted_job_by_id_emp_id($id,$emp_id){
		$this->db->where('ID', $id);
		$this->db->where('employer_ID', $emp_id);
		$return = $this->db->delete('pp_post_voulnteer');
		return $return;
	}
	...
	public function get_posted_voulnteer_by_id($id) {
       $Q = $this->db->query("CALL get_posted_voulnteer_by_id($id)");
        if ($Q->num_rows > 0) {
            $return = $Q->row();
        } else {
            $return = 0;
	...
	public function get_posted_voulnteer_by_job_id($job_id) {
        $this->db->select('pp_post_voulnteer.*, pp_employers.email, pp_employers.first_name');
        $this->db->from('pp_post_voulnteer');
		$this->db->join('pp_employers', 'pp_post_voulnteer.employer_ID = pp_employers.ID', 'inner');
        $this->db->where('pp_post_voulnteer.ID', $job_id);
		$Q = $this->db->get();
        if ($Q->num_rows > 0) {
            $return = $Q->row();
        } else {
	...
	public function get_active_posted_voulnteer_by_id($id) {
       $Q = $this->db->query("CALL get_active_posted_voulnteer_by_id($id)");
        if ($Q->num_rows > 0) {
            $return = $Q->row();
        } else {
            $return = 0;
	...
	public function get_posted_job_by_id_voulnteer_id($id,$employer_id) {
       $Q = $this->db->query("CALL get_posted_job_by_id_voulnteer_id($id,$employer_id)");
        if ($Q->num_rows > 0) {
            $return = $Q->row();
        } else {
            $return = 0;
	...
	public function get_posted_voulnteer_by_employer_ID($employer_id, $per_page, $page) {
       $Q = $this->db->query("CALL get_posted_voulnteer_by_employer_id($employer_id, $page, $per_page)");
        if ($Q->num_rows > 0) {
            $return = $Q->result();
        } else {
            $return = 0;
	...
	public function get_active_deactive_posted_voulnteer_by_company_id($company_id, $per_page, $page) {
       $Q = $this->db->query("CALL get_active_deactive_posted_voulnteer_by_company_id($company_id, $page, $per_page)");
        if ($Q->num_rows > 0) {
            $return = $Q->result();
        } else {
            $return = 0;
	...
	public function count_active_opened_voulnteer_by_company_id($company_id) {
		$Q = $this->db->query("CALL count_active_opened_voulnteer_by_company_id(".$company_id.")");	
		 if ($Q->num_rows > 0) {
            $return = $Q->row('total');
        } else {
            $return = 0;
	...
	//Search
	public function get_searched_voulnteer($param,$param2='', $per_page, $page) {
       $Q = $this->db->query('CALL ft_search_voulnteer("'.$param.'", "'.$param2.'", '.$page.', '.$per_page.')');
        if ($Q->num_rows > 0) {
            $return = $Q->result();
        } else {
            $return = 0;
...
");
	...
	public function count_searched_voulnteer_records($param,$param2) {
		$Q = $this->db->query('CALL count_ft_search_voulnteer("'.$param.'","'.$param2.'")');	
		 if ($Q->num_rows > 0) {
            $return = $Q->row('total');
        } else {
            $return = 0;
